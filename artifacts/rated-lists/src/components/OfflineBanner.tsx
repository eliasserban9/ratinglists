...
function useAnimatedMount(show: boolean) {
  ...
  useEffect(() => {
    ...
    if (show) {
      setMounted(true);
      timer.current = setTimeout(() => setVisible(true), 16);
    } else {
      setVisible(false);
      timer.current = setTimeout(() => setMounted(false), TRANSITION_MS + 40);
    }
...
export function OfflineBanner() {
  ...
  const banner = useAnimatedMount(showBanner);
  const dot    = useAnimatedMount(showDot);
  const online = useAnimatedMount(showOnline);
      ...
      {dot.mounted && (
        <button
          ...
          style={{
            ...
            transform: dot.visible ? "scale(1)" : "scale(0.4)",
            transition: `opacity ${TRANSITION_MS}ms ease, transform ${TRANSITION_MS}ms cubic-bezier(0.34,1.56,0.64,1)`,
          }}
        >
          !
        </button>
      ...
      {online.mounted && (
        <div
          ...
          style={{
            ...
            pointerEvents: "none",
            opacity: online.visible ? 1 : 0,
            transform: online.visible ? "translateY(0)" : "translateY(-100%)",
            transition: `opacity ${TRANSITION_MS}ms ease, transform ${TRANSITION_MS}ms cubic-bezier(0.4,0,0.2,1)`,
          }}
        >
          <span>✓</span>
